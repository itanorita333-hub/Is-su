...
async function sendInteractiveButtons(sock, jid, payload, options = {}) {
    ...
    const mediaField = ['image', 'video', 'audio', 'document', 'sticker'].find(field => payload?.[field] != null)

    if (mediaField) {
        const hasButtons = nativeButtons.length > 0
        const mediaPayload = {
            [mediaField]: payload[mediaField]
        }

        if (!hasButtons && payload?.caption) {
        ...
        if (payload?.ptt !== undefined) mediaPayload.ptt = payload.ptt
        if (payload?.contextInfo) mediaPayload.contextInfo = payload.contextInfo

        await sock.sendMessage(jid, mediaPayload, options)
...
function extractInteractiveResponseMetadata(message) {
    ...
    if (selectedLegacy) {
        return { id: String(selectedLegacy), url: '', text: '' }
    }
    ...
    try {
        ...
        const url = parsed.url
            ...
            || parsed.target_url
            || parsed.targetUrl
            || parsed.selected_row?.url
            || parsed.selectedRow?.url
            || ''
...
function extractInteractiveResponseId(message) {
    return extractInteractiveResponseMetadata(message).id
}
...
module.exports = {
    ...
    extractInteractiveResponseMetadata
}
