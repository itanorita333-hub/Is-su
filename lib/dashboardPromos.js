...
async function sendConfiguredPromoMessage(sock, chatId, settings, config = {}) {
    ...
    const text = configuredText
        ...
        : String(fallbackText || '')
    const configuredButtons = parseButtons(settings?.[buttonsKey])
    const buttons = configuredButtons.length ? configuredButtons : (Array.isArray(defaultButtons) ? defaultButtons : [])
