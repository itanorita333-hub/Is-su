 ...
 */
require('./settings')
const { installLogInterceptors } = require('./lib/installLogInterceptors')
...
// Single-instance lock to avoid duplicate sockets causing session conflicts.
const BOT_LOCK_FILE = path.join(__dirname, '.bot-instance.lock')
...
function releaseBotLock() {
    try {
        const ownerPid = Number(fs.readFileSync(BOT_LOCK_FILE, 'utf8'))
        if (ownerPid === process.pid) fs.unlinkSync(BOT_LOCK_FILE)
    } catch {}
}
...
function acquireBotLock() {
    try {
        ...
        let existingPid = NaN
        try {
            existingPid = Number(fs.readFileSync(BOT_LOCK_FILE, 'utf8'))
        } catch {}

        if (Number.isFinite(existingPid) && isPidAlive(existingPid)) {
            console.error(chalk.red(`❌ Another bot instance is already running (PID ${existingPid}). Exiting this process.`))
            process.exit(1)
        }

        // Stale lock from crashed process: replace it.
...
const qrRuntimeState = {
    lastQrRaw: null,
    lastQrLogAt: 0,
}

function readQrStateFile() {
    try {
        return JSON.parse(fs.readFileSync('./data/qrState.json', 'utf8'))
    } catch {
        return {}
    }
}

function writeQrState(nextState, { preserveQr = false } = {}) {
    try {
        const previous = preserveQr ? readQrStateFile() : {}
        const merged = preserveQr
            ? {
                ...previous,
                ...nextState,
                qr: nextState.qr || previous.qr || null,
                qrUpdatedAt: nextState.qr ? nextState.timestamp : previous.qrUpdatedAt || null,
            }
            : nextState

        fs.writeFileSync('./data/qrState.json', JSON.stringify(merged))
    } catch {}
}
...
// Writes live account/profile info to disk so the dashboard (a separate
// process when SKIP_DASHBOARD is set) can read it — it has no access to this
// process's in-memory socket, only to files.
async function refreshBotInfoFile(sock) {
    try {
        const selfJid = sock.decodeJid(sock.user?.id || '')
        let profilePic = null
        try {
            if (selfJid) profilePic = await sock.profilePictureUrl(selfJid, 'image')
        } catch (_) {}
...
async function startXeonBotInc() {
    ...
    XeonBotInc.ev.on('connection.update', async (s) => {
        ...
        if (qr) {
            const isNewQr = qr !== qrRuntimeState.lastQrRaw
            const now = Date.now()
            if (isNewQr || (now - qrRuntimeState.lastQrLogAt) > 30000) {
                console.log(chalk.yellow(isNewQr ? '📱 QR Code generated. Please scan with WhatsApp.' : '📱 QR Code refreshed. Please scan with WhatsApp.'))
                qrRuntimeState.lastQrLogAt = now
            }
            qrRuntimeState.lastQrRaw = qr

            try {
                const qrDataUrl = await QRCode.toDataURL(qr, { margin: 1, width: 256 })
                writeQrState({ status: 'pending', qr: qrDataUrl, timestamp: Date.now() }, { preserveQr: true })
            } catch (e) { console.error('Error saving QR state:', e.message) }
        }
        
        if (connection === 'connecting') {
            console.log(chalk.yellow('🔄 Connecting to WhatsApp...'))
            writeQrState({ status: qr ? 'pending' : 'connecting', timestamp: Date.now() }, { preserveQr: true })
        }
        
        if (connection == "open") {
            qrRuntimeState.lastQrRaw = null
            qrRuntimeState.lastQrLogAt = 0
            writeQrState({ status: 'connected', qr: null, qrUpdatedAt: null, timestamp: Date.now() })
            console.log(chalk.magenta(` `))
            console.log(chalk.yellow(`🌿Connected to => ` + JSON.stringify(XeonBotInc.user, null, 2)))

            await refreshBotInfoFile(XeonBotInc)

            // Dashboard runs in a separate process (SKIP_DASHBOARD mode) and can't
            // see this process's in-memory socket, so keep botInfo.json fresh on
            // an interval too — otherwise the sidebar profile picture only ever
            // updates at connection time and can drift from the real WhatsApp photo.
            if (global.__profilePicRefreshInterval) clearInterval(global.__profilePicRefreshInterval)
            global.__profilePicRefreshInterval = setInterval(() => {
                refreshBotInfoFile(XeonBotInc)
            }, 5 * 60 * 1000)
        ...
        if (connection === 'close') {
            ...
            const nextQrStatus = statusCode === 408 ? 'qr-expired' : 'disconnected'
            writeQrState({ status: nextQrStatus, timestamp: Date.now() }, { preserveQr: statusCode === 408 })
            global.botSocket = null
            if (global.__profilePicRefreshInterval) {
                clearInterval(global.__profilePicRefreshInterval)
                global.__profilePicRefreshInterval = null
            }
