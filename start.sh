...
LOCK_FILE="/tmp/knightbot-start.lock"
exec 200>"$LOCK_FILE"
if ! flock -n 200; then
  echo "⚠️ Another start.sh instance is already running. Exiting this one."
  exit 0
fi

# ── Clean slate: kill any leftover bot/dashboard processes from a previous run ──
# Without this, a workflow restart can leave the old "node index.js" process
# alive alongside the new one. Both then fight over the same WhatsApp session,
# causing repeated "conflict" disconnects (status 440) where the bot goes
# offline every few seconds and stops responding to commands.
pkill -9 -f "node index.js" 2>/dev/null
pkill -9 -f "node -e require\('\./dashboard'\)" 2>/dev/null
sleep 1
