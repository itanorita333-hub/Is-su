	...
	try {
        ...
        try {
            if (state.type === 'sticker') {
                await sock.sendMessage(chatId, { sticker: fs.readFileSync(assetPath) });
                return;
            }
            const payload = {};
...
async function mentionToggleCommand(sock, chatId, message, args, isOwner) {
	if (!isOwner) return sock.sendMessage(chatId, { text: 'Only Owner or Sudo can use this command.' });
	const onoff = (args || '').trim().toLowerCase();
	if (!onoff || !['on','off'].includes(onoff)) {
		return sock.sendMessage(chatId, { text: 'Usage: .mention on|off' });
	}
	const state = loadState();
	state.enabled = onoff === 'on';
	saveState(state);
	return sock.sendMessage(chatId, { text: `Mention reply ${state.enabled ? 'enabled' : 'disabled'}.` });
}
...
async function setMentionCommand(sock, chatId, message, isOwner) {
	...
	if (!qMsg) return sock.sendMessage(chatId, { text: 'Reply to a message or media (sticker/image/video/audio/document).' });
	...
	else return sock.sendMessage(chatId, { text: 'Unsupported. Reply to text/sticker/image/video/audio/document.' });

	// Download or capture text
	if (type === 'text') {
		buf = Buffer.from(qMsg.conversation || qMsg.extendedTextMessage?.text || '', 'utf8');
		if (!buf.length) return sock.sendMessage(chatId, { text: 'Empty text.' });
	} else {
		try {
			...
			return sock.sendMessage(chatId, { text: 'Failed to download media.' });
		}
	}

	// Size limit 1MB
	if (buf.length > 1024 * 1024) {
		return sock.sendMessage(chatId, { text: 'File too large. Max 1 MB.' });
	}
	...
	try { fs.writeFileSync(outPath, buf); } catch (e) {
		...
		return sock.sendMessage(chatId, { text: 'Failed to save file.' });
	}
	...
	saveState(state);
	return sock.sendMessage(chatId, { text: 'Mention reply media updated.' });
}
