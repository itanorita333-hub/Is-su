...
async function urlCommand(sock, chatId, message) {
    try {
        ...
        if (!media) {
            await sock.sendMessage(chatId, { text: 'Send or reply to a media (image, video, audio, sticker, document) to get a URL.' });
            return;
        }
        ...
        if (!url) {
            await sock.sendMessage(chatId, { text: 'Failed to upload media.' });
            return;
        }
