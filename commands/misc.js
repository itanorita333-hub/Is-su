...
async function handleHeart(sock, chatId, message) {
    try {
        ...
        await sock.sendMessage(chatId, { image: Buffer.from(response.data) });
    } catch (error) {
        console.error('Error in misc heart:', error);
        await sock.sendMessage(chatId, { text: '❌ Failed to create heart image. Try again later.' });
    }
}
    ...
    try {
        switch (sub) {
            ...
            case 'its-so-stupid': {
                ...
                if (!dog) {
                    await sock.sendMessage(chatId, { text: 'Usage: .misc its-so-stupid <text>' });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            case 'namecard': {
                ...
                if (!username || !birthday) {
                    await sock.sendMessage(chatId, { text: 'Usage: .misc namecard username|birthday|description(optional)' });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            case 'oogway2': {
                ...
                if (!quote) {
                    await sock.sendMessage(chatId, { text: `Usage: .misc ${sub} <quote>` });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            case 'tweet': {
                ...
                if (!displayname || !username || !comment) {
                    await sock.sendMessage(chatId, { text: 'Usage: .misc tweet displayname|username|comment|theme(optional light/dark)' });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            case 'youtube-comment': {
                ...
                if (!username || !comment) {
                    await sock.sendMessage(chatId, { text: 'Usage: .misc youtube-comment username|comment' });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            default:
                await sock.sendMessage(chatId, { text: 'Usage: .misc <heart|horny|circle|lgbt|lesbian|nonbinary|pansexual|transgender|lied|lolice|simpcard|tonikawa|its-so-stupid <text>|namecard u|b|d?|nobitches <text>|oogway <q>|oogway2 <q>|tweet dn|un|c|theme?|youtube-comment un|c>' });
                break;
        }
    } catch (error) {
        console.error('Error in misc command:', error);
        await sock.sendMessage(chatId, { text: '❌ Failed to generate image. Check your parameters and try again.' });
    }
}
