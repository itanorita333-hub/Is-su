...
async function scheduleCommand(sock, chatId, message, rawText, senderId) {
    ...
    if (sub === 'buat' || sub === 'add') {
        ...
        if (!parsed.ok) {
            await sock.sendMessage(chatId, { text: parsed.error });
            return;
        }
    ...
    if (sub === 'list') {
        if (chatId.endsWith('@g.us')) {
            ...
            if (!schedules.length) {
                await sock.sendMessage(chatId, { text: 'Tiada scheduled chat untuk group ini.' });
                return;
            }
        ...
        if (!allSchedules.length) {
            await sock.sendMessage(chatId, { text: 'Tiada scheduled chat lagi.' });
            return;
        }
    ...
    if (sub === 'delete' || sub === 'del' || sub === 'remove') {
        const idArg = parts[2];
        if (!idArg) {
            await sock.sendMessage(chatId, { text: 'Guna: `.schedule delete <id>`' });
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: `✅ Jadual #${result.removed.id} berjaya dipadam.` });
        return;
    }
    ...
    if (sub === 'clear') {
        ...
        await sock.sendMessage(chatId, { text: `✅ ${result.removedCount} jadual dipadam untuk chat ini.` });
        return;
    }
