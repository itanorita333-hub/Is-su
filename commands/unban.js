...
async function unbanCommand(sock, chatId, message) {
    ...
    if (isGroup) {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: 'Please make the bot an admin to use .unban', ...channelInfo });
            return;
        }
        if (!isSenderAdmin && !message.key.fromMe) {
            await sock.sendMessage(chatId, { text: 'Only group admins can use .unban', ...channelInfo });
            return;
        }
    } else {
        const senderId = message.key.participant || message.key.remoteJid;
        ...
        if (!message.key.fromMe && !senderIsSudo) {
            await sock.sendMessage(chatId, { text: 'Only owner/sudo can use .unban in private chat', ...channelInfo });
            return;
        }
    }
    let userToUnban;
