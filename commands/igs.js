...
async function igsCommand(sock, chatId, message, crop = false) {
    try {
        ...
        if (!downloadData || !downloadData.data) {
            await sock.sendMessage(chatId, { text: '❌ Failed to fetch media from Instagram link.' });
            return;
        }
        // Raw items
        ...
        if (items.length === 0) {
            await sock.sendMessage(chatId, { text: '❌ No media found at the provided link.' });
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: 'Failed to create sticker from Instagram link.' });
    }
}
