...
async function tagNotAdminCommand(sock, chatId, senderId, message) {
    try {
        ...
        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: 'Only admins can use the .tagnotadmin command.' });
            return;
        }
        ...
        if (nonAdmins.length === 0) {
            await sock.sendMessage(chatId, { text: 'No non-admin members to tag.' });
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: 'Failed to tag non-admin members.' });
    }
}
