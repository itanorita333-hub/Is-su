...
async function ytmp3Command(sock, chatId, message) {
    try {
        ...
        if (isYtUrl(args)) {
            ...
            if (!top) {
                return await sock.sendMessage(chatId, { text: '❌ No results found. Try a different keyword.' });
            }
        ...
        if (videoThumb) {
            await sock.sendMessage(chatId, { image: { url: videoThumb }, caption: waitMsg });
        } else {
            await sock.sendMessage(chatId, { text: waitMsg });
        }
