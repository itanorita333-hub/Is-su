...
async function spotifyCommand(sock, chatId, message) {
    try {
        ...
        if (!audioUrl) {
            await sock.sendMessage(chatId, { text: 'No downloadable audio found for this query.' });
            return;
        }
        ...
        const caption = `🎵 ${r.title || r.name || 'Unknown Title'}\n👤 ${r.artist || ''}\n⏱ ${r.duration || ''}\n🔗 ${r.url || ''}`.trim();
         ...
         if (r.thumbnails) {
            await sock.sendMessage(chatId, { image: { url: r.thumbnails }, caption });
        } else if (caption) {
        ...
        await sock.sendMessage(chatId, { text: 'Failed to fetch Spotify audio. Try another query later.' });
    }
}
