...
async function setGroupDescription(sock, chatId, senderId, text, message) {
    ...
    if (!desc) {
        await sock.sendMessage(chatId, { text: 'Usage: .setgdesc <description>' });
        return;
    }
    try {
        ...
        await sock.sendMessage(chatId, { text: '❌ Failed to update group description.' });
    }
}
...
async function setGroupName(sock, chatId, senderId, text, message) {
    ...
    if (!name) {
        await sock.sendMessage(chatId, { text: 'Usage: .setgname <new name>' });
        return;
    }
    try {
...
async function setGroupPhoto(sock, chatId, senderId, message) {
    ...
    if (!imageMessage) {
        await sock.sendMessage(chatId, { text: 'Reply to an image/sticker with .setgpp' });
        return;
    }
    try {
        ...
        await sock.sendMessage(chatId, { text: '✅ Group profile photo updated.' });
    } catch (e) {
        await sock.sendMessage(chatId, { text: '❌ Failed to update group profile photo.' });
    }
}
