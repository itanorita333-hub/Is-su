...
installLogInterceptors({ logFile: path.join(__dirname, 'data', 'bot.log') })
...
async function getLiveProfilePic(account) {
    // Dashboard runs as a separate OS process from the bot (index.js) when
    // SKIP_DASHBOARD is set — global.botSocket lives in the bot's process
    // memory and is never visible here, so a live in-process fetch is a
    // no-op. The bot itself keeps data/botInfo.json's profilePic fresh
    // instead (on connect + every 5 minutes); callers should use that value.
...
app.get('/api/status', async (req, res) => {
    ...
    // The bot (index.js) runs in its own OS process when SKIP_DASHBOARD is
    // set, so global.botSocket here is always null — isLiveSocketConnected
    // can only ever be true if dashboard and bot share a process. Fall back
    // to the file-based qrState written directly by the bot's
    // connection.update 'open' handler, which works across processes.
    // (Don't also require creds.registered — some restored/legacy sessions
    // never persist that flag even while genuinely connected.)
    const connected = isLiveSocketConnected(sock) || qrState.status === 'connected'
    const botInfo = readJSON('./data/botInfo.json', {})
    const accountRaw = sock?.user || (botInfo?.id ? { id: botInfo.id, name: botInfo.name } : (creds?.me || null))
    ...
    // botInfo.profilePic is kept fresh by the bot process itself (on connect
    // + every 5 min) — that's the reliable cross-process source of truth.
    // liveProfilePic only has a chance to work when dashboard and bot share
    // a process (SKIP_DASHBOARD unset).
    ...
    res.json({
        ...
        sessionReady,
        account,
        accountName: account?.name || settings.botName || 'Bot',
